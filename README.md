...
```sh
curl -u clientId:secret -X POST localhost:9191/oauth/token\?grant_type=password\&username=user\&password=pass
...
```sh
curl -u clientId:secret -X POST 'http://localhost:9191/oauth/token?grant_type=refresh_token&refresh_token=<refresh_token>'
